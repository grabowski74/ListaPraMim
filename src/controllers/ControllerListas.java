...
/**
	public String geraAutomaticaUltimaLista(String dataAtual) {
		for (ListaDeCompras lista : mapaDasListas.values()) {
			if (lista.getID() == this.id - 1) {
				mapaDasListas.put("Lista automatica 1 " + dataAtual, lista);
				mapaDasListas.get("Lista automatica 1 " + dataAtual).setID(id++);
				break;
			}
		}
...
/**
	public String geraAutomaticaItem(String descritorItem, String dataAtual) {
		...
		for (ListaDeCompras lista : mapaDasListas.values()) {
			if (lista.getID() == cont) {
				mapaDasListas.put("Lista automatica 2 " + dataAtual, lista);
				mapaDasListas.get("Lista automatica 2 " + dataAtual).setID(id++);
				break;
			}
		}
...
**/
	public String geraAutomaticaItem(String descritorItem, String dataAtual) {
		...
		if(c == false) {
			throw new Error("Erro na geracao de lista automatica por item: nao ha compras cadastradas com o item desejado.");
		}
		for (ListaDeCompras a : mapaDasListas.values()) {
			if (a.getID() == cont) {
...
/**
	public String geraAutomaticaItensMaisPresentes(Collection<Item> itens, String dataAtual) {
		List<Item> itensPresentes = new ArrayList<>();
		for (Item item : itens) {
			int i = 0;
			int media = 0;
			for (ListaDeCompras lista : mapaDasListas.values()) {
			...
			if (i!=0) {
				media = (int) Math.floor(media / i);
				item.setMedia(media);
			}
			

			if (i > (mapaDasListas.values().size())) {
		...
		for (Item item : itensPresentes) {
			...
			adicionaCompraALista("Lista automatica 3 " + dataAtual, item.getMedia(), item);
		}
